...
Menu, Tray, Icon, brightness.ico,,1

; [[[===== Functions =====]]]
; ----- Auto-click a button based on its text contents (optional) -----
ClickButtonByText(WinTitleIn, BtnTextIn) {
    WinTitle = %WinTitleIn%      ; the window's title
    BtnText = %BtnTextIn%        ; the button's text
...
WinWaitActive, Boot Camp Control Panel, ,20
if ErrorLevel
{
...
else
{
    ...
    ; ----- Step 3b (Option 1) - Tabs-and-spaces solution (Faster) -----
    ;Send {Tab}
    ;Send {Space}
    ;Send {Tab}
    ;Send {Tab}
    ...
    ; ----- Step 3b (Option 2) - Button names solution (Safer: Checks if boxes are checked) -----
    ; ~ Refresh window ~
    WinHide Boot Camp Control Panel
    WinShow Boot Camp Control Panel
    ...
    ; ----- (Option 1) - Click button by name (Unreliable - Can add pauses to make more reliable) -----
    ;ClickButtonByText("Boot Camp Control Panel", "&Apply") ; too unreliable
    ;ClickButtonByText("Boot Camp Control Panel", "OK")

    ; ----- (Option 2) - Tabs-and-spaces solution (Fast & reliable) -----
    Send {Tab}
    Send {Tab}
    Send {Tab}
    Send {Space}
...
F1:: BrightnessSetter.SetBrightness(-5)
F2:: BrightnessSetter.SetBrightness(+5)
F7::Send {Media_Prev}
