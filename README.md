...
Reboot after install. Each time you log in, **Task Scheduler** runs the `.exe`. (You should see the **Boot Camp Control Panel** pop up briefly while it does its thing.) 
...
If you want to tweak it yourself, it's built off an [AutoHotkey](https://www.autohotkey.com/) `.ahk` script which can be found in the repo. In order to run with Admin rights (to prevent the UAC prompt when opening the **Boot Camp Control Panel**), the `.exe` is run on login using a **Task Scheduler** task with highest privileges.
